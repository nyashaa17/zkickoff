...
export async function GET(
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch streaming buttons layout from remote server: ${res.status}`);
    }
    ...
    while ((match = linkRegex.exec(html)) !== null) {
      ...
      if (url.startsWith('http') && !url.includes('facebook.com') && !url.includes('twitter.com') && !url.includes('whatsapp://')) {
        ...
        servers.push({
          ...
          name: `${label} (HD)`,
          embedUrl: `https://king.totalsportss.online/embed?fixture=${encodeURIComponent(matchId)}&stream=${streamStr}`
        });
        count++;
      }
    }
    ...
    if (servers.length === 0) {
      ...
      uniqueUrls.forEach((url, index) => {
        servers.push({
          ...
          embedUrl: `https://king.totalsportss.online/embed?fixture=${encodeURIComponent(matchId)}&stream=${index + 1}`
        });
      });
    }
    ...
    if (servers.length === 0) {
      servers.push({
        ...
        embedUrl: `https://king.totalsportss.online/embed?fixture=${encodeURIComponent(matchId)}&stream=1`
      });
    }
    ...
    const fallbackServers = [
      ...
      },
      {
        ...
        embedUrl: `https://king.totalsportss.online/embed?fixture=${encodeURIComponent(matchId)}&stream=2`
      }
    ];
